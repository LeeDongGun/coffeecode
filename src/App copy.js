...
export default function CoffeeGame() {
  ...
  const startSpinning = () => {
    // Start from a random position to make it feel more random
    let currentIndex = Math.floor(Math.random() * players.length);
    let speed = 100; // 시작 속도 (ms)
    let iterations = 0;
    const maxIterations = Math.floor(Math.random() * players.length) + 1 + 20; // 1~참가자 수 사이의 랜덤 값 + 20
    ...
    const spin = () => {
      ...
      // 점점 느려지는 효과
      if (iterations > 15) { // Slow down more towards the end
        speed += 30;
      ...
      if (iterations < maxIterations) {
        ...
        // The currently highlighted player is the winner.
        setTimeout(() => {
          setWinner(players[currentIndex]);
          setIsSpinning(false);
          setCurrentHighlight(-1);

          // 게임 결과 저장
          if (!dontRecord) {
            saveGameResult(players[currentIndex]);
          }
        }, 1000);
      }
    };
